...
GPU_TEST_CASE("compute-indirect", D3D12 | Vulkan | CUDA | Metal)
{
...
GPU_TEST_CASE("compute-indirect-zero", D3D12 | Vulkan | CUDA | Metal)
{
    ComPtr<IShaderProgram> program;
    REQUIRE_CALL(loadProgram(device, "test-compute-indirect", "computeMain", program.writeRef()));
...
GPU_TEST_CASE("compute-indirect-offset", D3D12 | Vulkan | CUDA | Metal)
{
    ComPtr<IShaderProgram> program;
    REQUIRE_CALL(loadProgram(device, "test-compute-indirect", "computeMain", program.writeRef()));
